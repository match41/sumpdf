...
class CCITTFaxStream : public StreamFilter
{
public:

	CCITTFaxStream(StreamFilter *strA, int encodingA, GBool endOfLineA,
...
private:
	...
	int lookChar();
	int getChar() { int c = lookChar(); buf = EOF; return c; }
...
CCITTFaxStream::CCITTFaxStream(StreamFilter *strA, int encodingA, GBool endOfLineA,
			       GBool byteAlignA, int columnsA, int rowsA,
			       GBool endOfBlockA, GBool blackA):
    m_src(strA) {
  encoding = encodingA;
