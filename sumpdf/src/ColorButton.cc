...
void ColorButton::Initialize(QToolButton* box)
{
	box->setPopupMode( QToolButton::MenuButtonPopup );
	box->setMenu(
		OnCreateColorMenu( SLOT( OnColorChanged() ), Qt::black ) );
	box->setIcon(
...
{
	QAction *color_action = qobject_cast<QAction *>( sender() );
	this->setIcon(
		OnCreateColorButtonIcon(
		":/images/textpointer.png",
		qVariantValue<QColor>( color_action->data() ) ) );
	emit clicked( qVariantValue<QColor>( color_action->data() ) );
}

void ColorButton::OnSetColor( )
{
...
QMenu *ColorButton::OnCreateColorMenu( const char *slot, QColor default_color )
{
	QList<QColor> colors;
...
QIcon ColorButton::OnCreateColorButtonIcon( 
...
QIcon ColorButton::OnCreateColorIcon( QColor color )
{
